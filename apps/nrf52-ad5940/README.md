...
PT3 time-domain DSP control requires controller firmware **V34 or later**, which advertises `PT3_DSP`. Controller **V35 or later** additionally advertises `PT3_CAL_DFT` and accepts an RTIA-calibration `dft_num` of 256, 512, 1024, 2048, or 4096 points. Controller **V52 or later** additionally advertises `PT3_RTIA_SELECT`: the guarded choices are 5 kΩ, 10 kΩ, and 20 kΩ HSTIA only. The ninth `CFG,PT3` field carries the nominal HSTIA resistance and the fourteenth `CFG,PT3P` field carries the same setting for sequencer pulses. Firmware calibrates the selected RTIA before every RUN, reports nominal plus calibrated resistance, and the GUI stores the requested nominal value beside unchanged B1/B2 data in CSV. 40 kΩ and above are deliberately unavailable in PT3 because PGA ×9 with the external ±5.5 µA guard may not retain ADC headroom. Legacy command forms remain deterministic at 10 kΩ. Controller V30 or later reports a rejected RTIA calibration as `@ERR,PT3_CAL,LIB=<error>,SPI=<status>,RTIANOM=<ohm>,RTIA=<ohm>`; the GUI preserves that raw line and explains that the requested DUT DAC setpoints were not enabled.
