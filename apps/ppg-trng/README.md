...
The `Value` control can show ADC code or a bias-corrected current calculation. Normal PPG `G`/`I`/`R` samples arrive from the firmware as signed `LED - ambient` deltas; when a bias value is available, the GUI plots `bias + delta` so the y-axis is in ADC-code units. If no bias is set, the caption changes to `PPG delta | set bias` instead of labeling the delta as ADC code. Press `Measure bias` while the selected ADC input is at the bias condition, then `Current` displays `(adc - bias) / 8192 * 1.8`. CSV export keeps the original ADC code and bias code with the converted value.
