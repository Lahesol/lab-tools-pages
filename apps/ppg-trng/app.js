const els = {
  ...
  entropyBitLimit: document.querySelector('[id="90bBitLimit"]'),
  entropyTupleK: document.querySelector('[id="90bTupleK"]'),
  entropyDataClass: document.querySelector('[id="90bDataClass"]'),
  entropySourceName: document.querySelector('[id="90bSourceName"]'),
  entropySourceAvailable: document.querySelector('[id="90bSourceAvailable"]'),
  entropySourceTestCount: document.querySelector('[id="90bSourceTestCount"]'),
  entropySourceDescription: document.querySelector('[id="90bSourceDescription"]'),
  entropyRunButton: document.querySelector('[id="90bRunButton"]'),
  entropyClearButton: document.querySelector('[id="90bClearButton"]'),
  entropyExportButton: document.querySelector('[id="90bExportButton"]'),
  entropyExportInputButton: document.querySelector('[id="90bExportInputButton"]'),
  entropySampleCount: document.querySelector('[id="90bSampleCount"]'),
  entropyDataStatus: document.querySelector('[id="90bDataStatus"]'),
  entropyValue: document.querySelector('[id="90bEntropyValue"]'),
  entropyIidStatus: document.querySelector('[id="90bIidStatus"]'),
  entropyRctStatus: document.querySelector('[id="90bRctStatus"]'),
  entropyAptStatus: document.querySelector('[id="90bAptStatus"]'),
  entropyResultsBody: document.querySelector('[id="90bResultsBody"]'),
  entropyHealthSummary: document.querySelector('[id="90bHealthSummary"]'),
  entropyRctDetail: document.querySelector('[id="90bRctDetail"]'),
  entropyAptDetail: document.querySelector('[id="90bAptDetail"]'),
  entropyIidDetail: document.querySelector('[id="90bIidDetail"]'),
  entropyWarning: document.querySelector('[id="90bWarning"]'),
};
...
function renderPufResults(result = state.pufResults) {
  ...
  if (!result.attack?.results?.length) {
    els.pufAttackResultsBody.innerHTML = `<tr><td colspan="6" class="puf-empty">Coordinate attack unavailable${result.attack?.reason ? `: ${result.attack.reason}` : ""}.</td></tr>`;
  } else {
    const individualRows = result.attack.results.flatMap((item) => (item.perSetResults || []).map((set) => `
      <tr><td>${set.setIndex}</td><td>${formatPufMetric(item.trainingPercent, 0)}%</td><td>${set.trainingPointCount.toLocaleString()}</td><td>${set.testPointCount.toLocaleString()}</td><td>${formatPufMetric(set.logisticAccuracy, 4)}%</td><td>${formatPufMetric(set.svmAccuracy, 4)}%</td></tr>
    `));
    els.pufAttackResultsBody.innerHTML = individualRows.length
      ? individualRows.join("")
      : '<tr><td colspan="6" class="puf-empty">No individual CRP subset results were returned.</td></tr>';
  }
...
function exportPufResultsCsv() {
  ...
  (result.attack?.results || []).forEach((item) => (item.perSetResults || []).forEach((set) => rows.push([
    "coordinate_attack", `set${set.setIndex}`, result.responseLength, "", formatPufMetric(result.uniformity.meanPercent, 6), formatPufMetric(result.entropy.mean, 9),
    ...
    result.intraHd.available ? formatPufMetric(result.intraHd.meanPercent, 6) : "", formatPufMetric(result.correlation.maximumAbsolute, 9), item.trainingPercent, set.trainingPointCount, set.testPointCount,
    formatPufMetric(set.logisticAccuracy, 9), "", formatPufMetric(set.svmAccuracy, 9), "",
    "", "", "", "", "", "", "", "",
  ].join(","))));
