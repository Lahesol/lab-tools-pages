(function () {
  ...
  async function loadBundledDfuPackage() {
    ...
    if (release?.available === false) {
      throw new Error(release.message || "Latest DFU package has not been published yet.");
    }
...
Ready for browser UART DFU upload.`
  ...
  async function checkDfuSupport() {
    if (typeof isConnected === "function" && !isConnected()) {
      setDfuStatus("Connect to the running app first.", "warn");
      return;
    }
    await sendCommand("DFU?");
    setDfuStatus("DFU? sent. Check the log for DFU,READY.", "ok");
  }

  async function enterDfuBootloader({ silent = false } = {}) {
    const connected = typeof isConnected === "function" ? isConnected() : Boolean(state.port);
    if (!connected) {
      if (!silent) setDfuStatus("Connect to the running app first.", "warn");
      return false;
    }
    if (!silent && !confirm("Reset the board into UART DFU bootloader mode?")) return false;
    setDfuStatus("Requesting bootloader entry...");
    await sendCommand("DFU");
    setDfuStatus("DFU command sent; waiting for bootloader.", "warn");
    if (typeof setDfuBootloaderMode === "function") setDfuBootloaderMode(true, "warn");
    await dfuSleep(300);
    if (typeof disconnectActiveTransport === "function" && (typeof isConnected !== "function" || isConnected())) {
      await disconnectActiveTransport().catch(() => {});
    } else if (state.port) {
  ...
  async function showDfuCommand() {
    ...
    const fileName = file?.name || analysis?.fileName || "Stimulation_2emg.hex";
...
.\\tools\\dfu\\uart_dfu_from_hex.ps1 ${switchPart} "C:\\path\\to\\${fileName}" -Port ${port} -Baud ${baud} -AppBaud 115200 -AppVersion ${appVersion} -TriggerAppDfu`;
    setDfuInfo(command);
...
${finalExecuteTimedOut ? "Reconnect Web Serial and run VER? to verify the updated application." : "The bootloader should reset into the updated application. Reconnect Web Serial and run VER?."}`
  ...
  async function uploadDfuPackageWithBrowser(pkg, sourceLabel) {
    ...
    const baud = Math.max(9600, Math.round(Number($("dfuBaud")?.value) || 230400));
    const connectedNow = typeof isConnected === "function" ? isConnected() : Boolean(state.port || state.reader || state.writer);
    const connectedApp = connectedNow;
    const connectedSilent = false;
    const releaseText = pkg.release?.version ? `\nFirmware: ${pkg.release.version}` : "";
    const prompt = connectedApp
      ? `This will reset the current app into UART DFU mode, close the app connection, then ask you to select the bootloader COM port.\n\nPackage: ${pkg.fileName}${releaseText}`
      : connectedSilent
